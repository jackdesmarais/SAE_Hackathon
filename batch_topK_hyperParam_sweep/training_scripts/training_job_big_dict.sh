#!/bin/bash
#SBATCH --output=./batch_topK_hyperParam_sweep/logs/V2/%x_qsub.sh.stout.%A.%a      # Output file
#SBATCH --error=./batch_topK_hyperParam_sweep/logs/V2/%x_qsub.sh.sterr.%A.%a      # Error file
...
python ./Train_SpliceAI_WG_SAE.py --act-size 32 \
                            ...
                            --dict-size ${dict_size} \
                            --batch-size 524288 \
                            --n-batches-to-dead 20 \
                            --top-k-aux ${topk_aux} \
                            --aux-penalty ${aux_penalty} \
                            --preload-data \
                            --exp-name "v2_mem_fixed_batchtopk_HP_sweep" \
